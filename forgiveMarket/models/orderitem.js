...
function getAllOrderItemByRow(page,rows,cb){
	orderitemModel.find({}).limit(Number(rows)).skip(Number((page-1)*rows)).populate({
		path: 'oid' 
	}).populate({
		path: 'gid' 
	}).populate({
...
module.exports.getAllOrderItemByRow = getAllOrderItemByRow;
